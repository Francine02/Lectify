  ...
  return (
    <>
      <div className="flex items-center justify-center">
        ...
        />
        <label htmlFor={name} className="ms-2 text-sm font-medium text-gray-900 d">
          {t('polity.agree')}
          <a
            rel="noopener noreferrer"
            href={
              i18n.language === 'pt'
                ? '/privacyPolicy/privacy-policy-pt.pdf'
                : '/privacyPolicy/privacy-policy-en.pdf'
            }
            target="_blank"
      ...
      {errors[name] && (
        <Error className="flex justify-center pt-1" text={errors[name]?.message?.toString()} />
      )}
    </>
  );
}
